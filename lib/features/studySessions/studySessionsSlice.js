...
export const updateStudySession = createAsyncThunk(
    ...
    async ({ sessionId, flashcardId, difficulty, reviewTime, currentFlashcardIndex }, { rejectWithValue }) => {
        try {
            ...
            const response = await fetch(`/api/studySessions/${sessionId}`, {
                ...
                body: JSON.stringify({ flashcardId, difficulty, reviewTime, currentFlashcardIndex }),
            });
...
export const getActiveStudySession = createAsyncThunk(
    ...
    async ({ userId, setId }, { rejectWithValue }) => {
        try {
            const response = await fetch(`/api/studySessions?userId=${userId}&setId=${setId}`);
    ...
    extraReducers: (builder) => {
        builder
            .addCase(startStudySession.fulfilled, (state, action) => {
                console.log('startStudySession:', action.payload);
                state.session = action.payload;
                state.status = "succeeded";
            })
            .addCase(resumeStudySession.fulfilled, (state, action) => {
                console.log('resumeStudySession:', action.payload);
                state.session = action.payload;
                state.status = "succeeded";
            })
            .addCase(endStudySession.fulfilled, (state, action) => {
                console.log('endStudySession:', action.payload);
                state.session = null;
                state.status = "succeeded";
            })
            .addCase(updateStudySession.fulfilled, (state, action) => {
                console.log('updateStudySession:', action.payload);
                state.session = action.payload;
                state.status = "succeeded";
            })
            .addCase(getActiveStudySession.fulfilled, (state, action) => {
                console.log('getActiveStudySession:', action.payload);
                state.session = action.payload;
                state.status = "succeeded";
            })
            .addCase(startStudySession.rejected, resumeStudySession.rejected, endStudySession.rejected, updateStudySession.rejected, getActiveStudySession.rejected, (state, action) => {
                state.status = "failed";
