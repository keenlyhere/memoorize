...
export const addFlashcard = createAsyncThunk(
    ...
    async (flashcardData, { dispatch, rejectWithValue }) => {
        try {
			...
			const { newFlashcard, updatedSet } = await response.json();

			// update set's flashcard count
			dispatch(updatedSetFlashcardCount(updatedSet));
...
export const removeFlashcard = createAsyncThunk(
	...
	async ({ flashcardId, userId }, { dispatch, rejectWithValue }) => {
		try {
			...
			// update set's flashcard count (-1)
			dispatch(updatedSetFlashcardCount(updatedSet));
	...
	extraReducers: (builder) => {
		builder
			...
			.addCase(getFlashcards.fulfilled, (state, action) => {
				...
				state.flashcardSetTitle = action.payload.setTitle;
