...
import { getAllStudySessions } from "@/lib/features/studySessions/studySessionsSlice";
import { getAllFlashcardSets } from "@/lib/features/flashcardSets/flashcardSetsSlice";

export default function Courses() {
	const dispatch = useAppDispatch();
	const currUser = useAppSelector((state) => state.user);
	const { courses } = useAppSelector((state) => state.courses);
	const { allFlashcardSets } = useAppSelector((state) => state.flashcardSets); // Adjusted to use allFlashcardSets
	...
	useEffect(() => {
		if (currUser && currUser.id !== null) {
			...
			dispatch(getAllFlashcardSets({ userId: currUser.id }));
			dispatch(getAllStudySessions({ userId: currUser.id }));
		}
	}, [currUser, dispatch]);

	useEffect(() => {
		console.log("allFlashcardSets", allFlashcardSets);
		// console.log('sessions', sessions)
		// console.log('courses', courses)
	}, [allFlashcardSets]);

	useEffect(() => {
		if (currUser && courses && allFlashcardSets) {
			setIsLoaded(true);
		}
	}, [currUser, courses, allFlashcardSets, setIsLoaded]);

	const calculateProgress = (course) => {
		const courseSets = allFlashcardSets.filter((set) => set.courseId === course.id); // Adjusted to use allFlashcardSets
		const totalSets = courseSets.length;
		const studiedSets = courseSets.filter((set) => sessions.some((session) => session.setId === set.id)).length;

		const progressPercentage = totalSets > 0 ? (studiedSets / totalSets) * 100 : 0;

		return { studiedSets, totalSets, progressPercentage };
	};
	...
	const handleDeleteClick = (courseId) => {
		...
		openModal(<DeleteConfirmation onConfirm={() => handleDelete(courseId)} onClose={closeModal} />);
	};
			...
			{isLoaded ? (
				<>
					...
					<div className="flex justify-between items-center mb-6">
						...
						<button
							onClick={() =>
								openModal(<AddCourseForm onClose={closeModal} userId={currUser.id} type="Course" />)
							}
					...
					<div className="w-full">
						<div className="flex flex-col w-full items-center justify-between pb-4">
							{courses && courses.length > 0 ? (
								courses.map((course) => {
									const { studiedSets, totalSets, progressPercentage } = calculateProgress(course);
											...
											>
												<div className="flex w-full items-center space-x-4 grow">
													...
													<div className="text-gray-500 pr-2">
														{/* Change color of check mark if user completed all sets */}
														<svg
															xmlns="http://www.w3.org/2000/svg"
															fill="none"
															viewBox="0 0 24 24"
															strokeWidth="2"
															stroke={`${
																progressPercentage === 100 ? "#00c89e" : "currentColor"
															}`}
															className="w-6 h-6"
														>
															<path
														...
														<div className="flex flex-col grow">
															{editingCourseId === course.id ? (
																<input
																	...
																	onBlur={() =>
																		handleTitleBlur(course.id, course.title)
																	}
																	onKeyDown={(e) => {
																		if (e.key === "Enter") {
															...
															<span className="text-sm text-gray-500">
																{studiedSets} of {course.setCount ? course.setCount : 0}{" "}
																sets studied
															</span>
								...
								<p className="dark:text-dark-gray">
									You have no courses. Click the &apos;+ Course&apos; button on the top right to add a
									course!
								</p>
							)}
						</div>
