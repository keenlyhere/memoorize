...
import { getFlashcards, removeFlashcard, updateFlashcard } from "@/lib/features/flashcards/flashcardsSlice";
...
export default function Set({ params }) {
  ...
  useEffect(() => {
	...
	const handleDeleteClick = (flashcardId) => {
		openModal(<DeleteConfirmation onConfirm={() => handleDelete(flashcardId)} onClose={closeModal} />);
	};
			...
			{isLoaded ? (
				<>
					<div className="flex justify-between items-center mb-6">
						<h1 className="text-xl font-medium text-dark-gray py-2">{ setTitle }</h1>
						<button
							onClick={() =>
								openModal(
									<AddFlashcardForm
						...
						{viewMode === "all" ? (
							<div className="flex flex-col w-full items-center justify-between py-4">
								{flashcards && flashcards.length > 0 ? (
													...
													<div className="flex flex-col grow">
														{editingFlashcardId === flashcard.id ? (
															<>
																...
																<textarea
																	value={editedAnswer}
																	onChange={(e) => handleFieldChange(e, "answer")}
																	onBlur={() =>
																		handleFieldBlur(
																			flashcard.id,
																			flashcard.question,
									...
									<p className="dark:text-dark-gray">
										You have no flashcards. Click the &apos;+ Flashcard&apos; button to add a
										flashcard!
									</p>
								)}
							</div>
							...
							<div className="w-full flex items-center justify-center">
								<div className="text-center p-6 bg-light-gray rounded-lg shadow-md">
									<p className="text-lg font-semibold text-dark-gray">Study Mode</p>
									{flashcards && flashcards.length > 0 ? (
										flashcards.map((flashcard, index) => (
											...
											>
												...
												>
													<p className="text-sm text-gray-500">{flashcard.answer}</p>
												</div>
											</div>
										))
									) : (
										<p className="dark:text-dark-gray">No flashcards available.</p>
									)}
								</div>
							</div>
						)}
